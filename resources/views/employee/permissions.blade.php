                ...
                <!-- /.box-header -->
                    ...
                    <div class="box">
                        ...
                        @if(auth()->user()->hasPermissionTo('permissions.create'))
                            <a class="btn btn-small btn-primary pull-right" href="#createPermission" data-toggle="modal"><i class="fa fa-plus"></i>&nbsp; {{__('Create Permission')}}</a>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-3">
                            <div class="box box-primary">
                                <div class="box-header">{{ucfirst($key)}} <input type="checkbox" id="checkAllPer" class="pull-right" onclick="checkPermissions(this)" {{($role->hasAnyPermission($svalue)) ? 'checked' : ''}}></div>
                                <div class="box-body">
                                    @foreach($svalue as $value)
                                        <p><input type="checkbox" name="permissions[]" value="{{$value->id}}" {{($role->hasPermissionTo($value->name)) ? 'checked' : ''}}> {{$value->label}}</p>
                                    @endforeach
                                </div>
                                <!-- /.box-body -->
                            </div>
